...
try:
    from ccPaper.core.data_schema import (
        FieldSolution, Charge, ModelParameters,
        ValidationResult, create_empty_solution
    )
except ImportError:
...
class BaseFieldModel(ABC):
    ...
    def __init__(self, model_name: ChargeType, dimension: DimensionType = "3D") -> None:
        """
        初始化基类
    ...
    def generate_training_data(
        ...
        if not self._charges or len(self._charges) == 0:
            ...
            # 修复：明确转换为整数
            n_local_int = int(0.8 * n_samples)
            n_global_int = n_samples - n_local_int
            ...
            for charge in self._charges:
                ...
                # 高斯分布采样 - 修复：确保整数除法
                samples_per_charge = max(1, n_local_int // len(self._charges))
                local = pos + np.random.normal(0, 0.3, (samples_per_charge, 3))
                local_points.append(local)

            global_points = np.random.uniform(-2, 2, (n_global_int, 3))
    ...
    def _get_allowed_parameter_keys(self) -> set:
        ...
        base_keys = {
            ...
            # 添加在实际应用中使用的参数
            'grid_size', 'bbox', 'timestamp', 'position',
            'ml_enabled', 'validation_level', 'cache_enabled',
            'mesh_resolution', 'boundary_type', 'material_type'
        }
        return base_keys
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        for charge in self._charges:
            charge_dict = {
                'position': tuple(float(x) for x in charge['position']),  # 修复：确保tuple of float
                'value': float(charge['value'])  # 修复：确保float类型
            }
        ...
        for key, value in self._parameters.items():
            serialized_params[key] = _serialize_numpy(value)
...
class CoulombModelMixin:
    ...
    def compute_coulomb_field(
        ...
        for charge in source_charges:
            q = float(charge['value'])  # 修复：明确转换为float
            pos_array = np.array(charge['position'], dtype=np.float64)
            pos = pos_array.astype(np.float64)  # 修复：确保float64类型
...
class ParameterValidationMixin:
    ...
    def validate_charge_range(
        ...
        """验证电荷量范围"""
        for i, charge in enumerate(charges):
            q_val = float(charge['value'])  # 修复：明确转换为float
            if not np.isfinite(q_val):
                return {
                    'is_valid': False,
                    'message': f"电荷 {i} 的值不是有限数: {q_val}",
                    'detail': {'charge_index': i, 'invalid_value': q_val}
                }
            if q_val < min_charge or q_val > max_charge:
                return {
                    'is_valid': False,
                    'message': f"电荷 {i} 超出允许范围 [{min_charge}, {max_charge}]: {q_val:.2e}",
                    'detail': {'charge_index': i, 'charge_value': q_val}
                }
    ...
    def validate_geometry_range(
        ...
        for key, value in parameters.items():
            ...
            if isinstance(value, (int, float)):
                value_float = float(value)  # 确保是float类型
                if not np.isfinite(value_float):
                    return {
                        'is_valid': False,
                        'message': f"参数 {key} 不是有限数: {value_float}",
                        'detail': {'param': key, 'value': value_float}
                    }
                if key in ['radius', 'length']:
                    if abs(value_float) > max_coord:
                        return {
                            'is_valid': False,
                            'message': f"参数 {key} 超出几何范围: |{value_float}| > {max_coord}",
                            'detail': {'param': key, 'value': value_float}
                        }
...
def test_base_model():
    ...
    # 测试1: 维度处理
    points_2d = np.array([[1.0, 0.0], [0.0, 1.0]])  # 修复：明确使用float
